...
const checkDebug = require('./debug-export.cjs');
...
(async () => {
    const work = fs.mkdtempSync(path.join(os.tmpdir(), 'studio-debug-export-'));
    ...
    try {
        ...
        await checkDebug(baseUrl, work);
        for (const script of ['viewer-lifecycle.cjs', 'typed-viewer.cjs']) {
            // Keep this process's HTTP server responsive while the child tests run.
            await new Promise((resolve, reject) => {
                const child = spawn(process.execPath, [path.join(__dirname, script)], {
                    cwd: root, env: { ...env, STUDIO_BASE_URL: baseUrl }, stdio: 'inherit', timeout: 300_000
                });
