...
(async () => {
    ...
    try {
        ...
        for (const order of ['first', 'last']) {
            const name = `animation-static-${order}.Item.Gbx`;
            const contributed = path.join(__dirname, 'Fixtures', name);
            assert.deepEqual(fs.readFileSync(path.join(fixtures, name)), fs.readFileSync(contributed),
                `${name}: generated bytes must match the native-validated contribution`);
            console.log(`Testing static-${order} through ${baseUrl}`);
            await run(process.execPath, ['Tests/Browser/animation-upload.cjs', contributed],
