...
It requires generated bytes to match the native-validated files in `../Fixtures`
and uploads those contributed files, so changes cannot silently bypass that gate.
...
order. Both exact contributed files have separately passed fresh TM2020 preload
and native resave; hashes and validation scope are in `../Fixtures/README.md`.
The unshaded fixtures have no collision surfaces. Neither this browser check nor
preload/resave proves in-game rendering, placement, authored constraint timing,
or resolution of the reported item-editor overwrite/save crash.
