...
const env = { ...process.env, DOTNET_PROCESSOR_COUNT: '2', DOTNET_ROLL_FORWARD: process.env.DOTNET_ROLL_FORWARD || 'Major' };
...
(async () => {
    ...
    try {
        ...
        if (!address || typeof address !== 'object') throw new Error('Failed to bind the test server.');
        ...
        await run(process.execPath, ['Tests/Browser/capability-gating.cjs', path.join(fixtures, 'light-capability.Item.Gbx')],
